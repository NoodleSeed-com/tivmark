...
| Managed chat chrome                  | High but intentionally bounded: panel surface/elevation/border/radius, launcher style/icon/size/status/effect, header mark/badge, composer icons/shape, and message treatments     | Solid softly elevated panel, stable chat launcher, single-brand header, clean pill composer, accent user messages, and plain assistant replies |
| Layout and behavior                  | High: floating/inline/drawer baseline, position, dimensions, offsets, density, mobile fullscreen, launcher/header/avatar/timestamp/powered-by visibility, and interaction behavior | Roomy public assistant, host-positioned signed-in drawer, fullscreen mobile, uncluttered transcript, and compact business confirmations        |
...
| Copy and entry points                | High: labels, loading/error/retry/sign-in/confirmation copy, locale/direction, and exact or generated starter prompts                                                              | Mark-specific states, public-to-signed-in copy, and five concise flagship prompts                                                              |
...
1. Open `https://tivmark.com` in light mode. Start with the stable “Ask Mark” launcher. Point out that its pill shape, chat icon, size, and intentionally motionless state are all Noodle presentation settings, not a hand-built button.
2. Open Mark. Show the single Tivmark identity in the header, generous panel proportions, Tivmark palette and typography, concise starter prompts, plain assistant replies, and distraction-free transcript. Avatar, timestamp, secondary mark, status badge, composer mark, and attribution visibility are all supported behavior or presentation choices.
...
- `apps/assistant/src/server.ts` declares the complete portable brand kit and a deliberately restrained managed presentation: no pulse, one expanded brand moment, softer chrome, concise copy, and a cleaner transcript.
