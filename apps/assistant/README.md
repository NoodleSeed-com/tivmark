...
```sh
noodle login
noodle deploy --org <org> --app tivmark-assistant
```

`noodle deploy` infers and saves the target on first run, so later deploys are just `noodle deploy`. (Optional: `noodle link --org <org> --app tivmark-assistant` saves the target up front; `noodle start` scaffolds + deploys in one step.)
...
This project was scaffolded with `--no-agents` (the repo already has its own agent conventions). To add
project-local Codex / Claude Code instructions later, run `noodle agents setup --write`.
