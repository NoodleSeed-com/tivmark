...
//
// The NoodleSeed broker authenticates with our client credential and presents a short-lived,
// platform-signed assertion (`subject_token`) of the signed-in user. We verify it against the
// platform JWKS, then mint a user-scoped Tivmark token via the same `issueAccessToken` the OAuth
// server uses — so downstream v1 API calls run as a real user principal and the API enforces its own
...
export default async function handler(
  ...
  if (!delegClientId || !delegClientSecret) {
    return oauthError(
      res,
      503,
  ...
  // --- verify the platform-signed assertion ---
  let userId: string;
  try {
